...
resource "azurerm_kubernetes_cluster" "example" {
  ...
  dns_prefix          = local.dns-prefix[terraform.workspace]
...
resource "azurerm_container_registry" "acr" {
  name                = local.acr-name[terraform.workspace]
  resource_group_name = data.azurerm_resource_group.example.name
  location            = data.azurerm_resource_group.example.location
  sku                 = var.acr["tier"]
  admin_enabled       = true
...
resource "azurerm_dns_a_record" "example" {
  ...
  name                = local.arecord-name[terraform.workspace]
...
resource "azurerm_mysql_server" "example" {
  name                = local.mysql-name[terraform.workspace]
  location            = var.mysql-location["location"]
  resource_group_name = data.azurerm_resource_group.example.name
...
resource "azurerm_mysql_firewall_rule" "example" {
  ...
  start_ip_address    = "188.27.84.62"
  end_ip_address      = "188.27.84.62"
}
